...
K = list(np.arange(-1, 1, 0.2))
#combs = list(combinations(K, 3))
combs = [[0.4, 0.6, 0.8], ]

II = 0
for p1, p2, p3 in combs:
    print(II, len(combs))
    II+=1
    ...
    while t < T:
        ...
        for car_idx, (car_r, car_c) in enumerate(car_pos):
            ...
            for ride_idx, ride in rides_idxs.items():
                ...
                possible, t_finish, car_arrival_dist, bonus = ride_info(t, car_r, car_c, ride, rdist, T)
                if possible:
                    choices.append((ride_idx, t_finish, car_arrival_dist, rdist, bonus))
            # select min t_finish
            if choices:
                #ride_idx, t_finish, car_arrival_dist, rdist, bonus = min(choices, key=lambda x: (x[2], x[1], (-x[3] -x[4])))
                ride_idx, t_finish, car_arrival_dist, rdist, bonus = min(choices, key=lambda x: (x[1]*p1 + x[2]*p2 + (x[3]+x[4])*p3))
                #ride_idx, t_finish, car_arrival_dist, rdist, bonus = min(choices, key=lambda x: x[2])
                #ride_idx, t_finish, car_arrival_dist, rdist, bonus = min(choices, key=lambda x: (x[1], (-x[3]-x[4])))
                #ride_idx, t_finish, car_arrival_dist, rdist, bonus = max(choices, key=lambda x: (x[3] + x[4], -x[1]))
